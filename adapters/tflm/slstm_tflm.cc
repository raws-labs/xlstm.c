...
TfLiteStatus SLstmPrepare(TfLiteContext* context, TfLiteNode* node) {
    ...
    // Gate accumulators: float on the f32 path, int32 on the INT8 one.
    // Sized for the wider of the two so Prepare does not have to care which
    // Eval will run.
    ...
    TF_LITE_ENSURE_OK(
        ...
        context->RequestScratchBufferInArena(
            context, 4 * op_data->hidden_size * elem,
...
TfLiteStatus SLstmEvalInt8(TfLiteContext* context, TfLiteNode* node,
                           const OpDataSLstm* op_data) {
    MicroContext* micro_context = GetMicroContext(context);
    ...
    params.R_scale = recurrent_weights->params.scale;
...
TfLiteStatus SLstmEval(TfLiteContext* context, TfLiteNode* node) {
    ...
    switch (input_type) {
        ...
        case kTfLiteInt8:
            return SLstmEvalInt8(context, node, op_data);
        default:
