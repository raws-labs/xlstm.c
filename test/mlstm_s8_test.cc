...
struct MlstmS8Setup {
    int8_t W_q[(4 * XLSTM_TEST_MAX_H + 2) * XLSTM_TEST_MAX_H]; /* max [(4*H+2), I] */
    int32_t b_q[4 * XLSTM_TEST_MAX_H + 2];        /* max [4*H+2] */
...
/* n and C are INT16 and read/written as strictly symmetric by mlstm_s8.c
 * (no zero-point term anywhere), so they are calibrated with
 * xlstm_quant_symmetric_s16 - see its comment in include/xlstm_quant.h.
 *
 ...
 * ratio table and how 4x was chosen.
 *
...
/* generate_reference.py's numpy replica calibrates n_quant/C_quant with
...
static_assert(kStateHeadroom == XLSTM_GENERATOR_HEADROOM,
              "kStateHeadroom (mlstm_s8_test.cc) must match "
 ...
 * the kernel, so they use xlstm_quant_symmetric_s16, not
...
static float EvalMlstmS8Case(const XlstmRefCase* tc, float* y_out,
    ...
    static int16_t C[XLSTM_TEST_MAX_H * XLSTM_TEST_MAX_H];
    static int16_t n_state[XLSTM_TEST_MAX_H];
...
static bool RunMlstmS8Case(const XlstmRefCase* tc) {
     ...
     * non-NULL - guarded the same way sLSTM's PrepareS8 already guards
