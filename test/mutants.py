...
asserts the suites FAIL. A defect the suites do not notice is an ESCAPE and
fails this run. Run it after any change to tolerances, bounds, or
test/generate_reference.py - that change class fails by making a gate stop
failing, which nothing else here detects.
...
# id, what, expect, backends, edits
MUTANTS = [
    ("A1", "activation drift y * 1.001", "pass", BOTH, drift("1.001")),
    ("A2", "activation drift y * 1.002", "fail", BOTH, drift("1.002")),
    ("A3", "activation drift y * 1.05", "fail", BOTH, drift("1.05")),
    ("B1", "final y zeroed at H=8, output[] correct", "fail", BOTH,
     after_eval("    if (H == 8) { for (i = 0; i < B * H; ++i) y[i] = 0; }\n",
                "    if (H == 8) { for (i = 0; i < B * H; ++i) y[i] = 0; }\n")),
    ("B2", "exit state zeroed", "fail", BOTH,
     after_eval("    for (i = 0; i < B * H; ++i) { c[i] = 0; n[i] = 0;"
                " m[i] = 0.0f; }\n",
                "    for (i = 0; i < B * H * H; ++i) C[i] = 0;\n"
                "    for (i = 0; i < B * H; ++i) n[i] = 0;\n"
                "    for (i = 0; i < B; ++i) m[i] = 0.0f;\n")),
    ("B3", "sub-maximum state elements zeroed", "fail", BOTH,
     after_eval(SUBMAX % "tt[0] = c; len[0] = B * H; tt[1] = n; len[1] = B * H;",
                SUBMAX % "tt[0] = C; len[0] = B * H * H; tt[1] = n; len[1] = B * H;")),
    ("C1", "state requantization drift 1.05x", "fail", BOTH,
    ...
    ("F1", "sLSTM H=17 channel 5 zeroed", "fail", BOTH,
     [(S8, SY, SY + "\n        if (H == 17 && i == 5) y_new = 0.0f;")]),
    ("F2", "sLSTM H=17 channel 6 zeroed", "fail", BOTH,
     [(S8, SY, SY + "\n        if (H == 17 && i == 6) y_new = 0.0f;")]),
]

FILES = sorted({f for m in MUTANTS for (f, _, _) in m[4]})
...
def main(backends):
    ...
    for rel, old, _ in [(f, o, n) for m in MUTANTS for (f, o, n) in m[4]]:
    ...
    try:
        for backend in backends:
            ...
            for mid, what, expect, on, edits in MUTANTS:
    ...
    print("\n%-3s %-*s %-7s %s"
          % ("id", width, "mutation", "expect", " ".join("%-10s" % b
                                                         for b in backends)))
    for mid, what, expect, _, _ in MUTANTS:
        print("%-3s %-*s %-7s %s"
    ...
    print("mutants: " + ("OK - every injected defect is caught, and the "
                         "portability margin holds" if not bad else
                         "FAILED - %d mutation(s) escaped or false-failed" % bad))
